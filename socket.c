...
void __attribute__((constructor)) hook_init(void)
{
	...
	pthread_t thread;
	plist temp = NULL;
	void* socket_driver = NULL;
	int(*plugin_register)(prps,int) = NULL;
	int(*plugin_exit)(void) = NULL;
	void* plugin_setup = NULL;
	if(!task){
		//初始化任务
		init_task();
		socket_driver = &(task->socket_driver_table);
		path = getenv("PLUGIN_PATH");
		if(path == NULL)
		{
			task->bEnablePlugin = 0;
			return;
		}
		...
		if(!task->plugin)
		{
			task->plugin = init_list();
			task->plugin->target = -1;
		}
		while(file = readdir(dir)) {
			// 判断是否为文件
			if (file->d_type == 8) {
				if((strlen(path) + strlen(file->d_name)) > 512)
				{
					printf("溢出\n");
					task->bEnablePlugin = 0;
					return;
				...
				if(plugin_register(&reg,count) != 0)
				{
					WARN_PRINT("插件不可用2");
					memset(buf,0,512);
					continue;
				...
				temp = init_list();
				temp->target = Malloc_Zero(sizeof(prs));
				if(reg.version >= SUPPORT_LOW_VERSION && reg.version <= CURRENT_VERSION){
					((pplugins)temp->target)->reg.version = reg.version;
					((pplugins)temp->target)->reg.author = Malloc_Zero(strlen(reg.author));

					memcpy(((pplugins)temp->target)->reg.author,
							reg.author,
							strlen(reg.author));
					((pplugins)temp->target)->ID = count;
					plugin_exit = dlsym(handle,"plugin_exit");
					if(!plugin_exit)
					{
						WARN_PRINT("插件不可用3");
						free(((pplugins)temp->target)->reg.author);
						free(temp->target);
						free(temp);
						memset(buf,0,512);
						continue;
					}
					plugin_setup = dlsym(handle,"plugin_setup");
					if(!plugin_setup)
					{
						WARN_PRINT("插件不可用4");
						free(((pplugins)temp->target)->reg.author);
					...
					//这里开启线程
					if(pthread_create(&thread,NULL,run,plugin_setup) != 0)
					{
						WARN_PRINT("线程开启失败");
						free(((pplugins)temp->target)->reg.author);
						free(temp->target);
					...
					((pplugins)temp->target)->thread = thread;
					((pplugins)temp->target)->handle = handle;
					((pplugins)temp->target)->finit = plugin_exit;
					printf(cGRN"加载成功==> %s \n"cRST,buf);
					add_list(task->plugin,temp);
					memset(buf,0,512);
					count++;
				}else
				{
					...
					if((strlen("[无法加载.so]==> version:") + strlen(buf) + 1) > 512)
					{
						ERROR_PRINT("栈溢出\n");
						return;
					}
					memset(buf,0,512);
					sprintf(buf,"[无法加载.so]==> version:%d 路径:%s",reg.version,t);
					PLUGIN_WARN_PRINT(buf);
					memset(buf,0,512);
					sprintf(buf,"最低版本: %d 最高版本: %d ",SUPPORT_LOW_VERSION,CURRENT_VERSION);
					PLUGIN_WARN_PRINT(buf);
					free(temp->target);
					memset(buf,0,512);
					plugin_exit();
...
int try_to_connect()
{
	...
	if(task->ncpc == -1)
	{
		...
		memset(task->sf,0,sizeof(scanfs));
		EXIT(-1);
	}
